...
namespace app
{
	...
	bool CGraphicPipelineModel::_initialize()
	{
		...
		// ルートシグネチャ生成
		{
			...
			if(NG_FAILED(err)) {
				NG_ERRLOG_C("GraphicPipelineModel", err, "DX12ルートシグネチャの生成に失敗しました.");
				return false;
			}
		}
		...
		// ディスクリプタヒープ生成
		{
			...
			if(NG_FAILED(err)) {
				NG_ERRLOG_C("GraphicPipelineModel", err, "DX12ディスクリプタヒープの生成に失敗しました.");
				return false;
			}
		}
		...
		// コンスタントバッファ生成
		{
			...
			if(NG_FAILED(err)) {
				NG_ERRLOG_C("GraphicPipelineModel", err, "DX12コンスタントバッファの生成に失敗しました.");
				return false;
			}
		...
		// シェーダー生成
		{
			...
			if(NG_FAILED(err = loader.LoadShaderFromFile(
				m_vs,
				L"../resource/shader/model_test.hlsl",
				"VSMain",
				"vs_5_0",
				0, true
				))) {
				NG_ERRLOG_C("GraphicPipelineModel", err, "DX12頂点シェーダーの生成に失敗しました.");
				return false;
			}
			// ピクセルシェーダー
			if(NG_FAILED(err = loader.LoadShaderFromFile(
				...
				NG_ERRLOG_C("GraphicPipelineModel", err, "DX12ピクセルシェーダーの生成に失敗しました.");
				return false;
			}
		}
		...
		// パイプラインステート生成
		{
			...
			D3D12_INPUT_ELEMENT_DESC inputElemDescs[] = {
				...
				{"NORMAL",		0, DXGI_FORMAT_R32G32B32_FLOAT,		12, D3D12_APPEND_ALIGNED_ELEMENT,	D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
				{"TEXCOORD",	0, DXGI_FORMAT_R32G32_FLOAT,		24, D3D12_APPEND_ALIGNED_ELEMENT,	D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
			};
			...
			if(NG_FAILED(err)) {
				NG_ERRLOG_C("GraphicPipelineModel", err, "DX12パイプラインステートの生成に失敗しました.");
				return false;
			}
		}
